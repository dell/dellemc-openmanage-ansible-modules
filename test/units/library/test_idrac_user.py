...
import pytest
import json
from ansible.modules.remote_management.dellemc import idrac_user
...
class TestIDRACUser(FakeAnsibleModule):
    ...
    def idrac_connection_user_mock(self, mocker, idrac_user_mock):
        idrac_conn_mock = mocker.patch('ansible.modules.remote_management.dellemc.idrac_user.iDRACRedfishAPI',
    ...
    def test_get_user_account_1(self, idrac_connection_user_mock, idrac_default_args, mocker):
        idrac_default_args.update({"state": "present", "new_user_name": "new_user_name",
                                   ...
                                   "sol_enable": True, "protocol_enable": True,
                                   "authentication_protocol": "SHA", "privacy_protocol": "AES"})
        mocker.patch("ansible.modules.remote_management.dellemc.idrac_user.iDRACRedfishAPI.export_scp",
                     return_value=MagicMock())
        mocker.patch("ansible.modules.remote_management.dellemc.idrac_user.iDRACRedfishAPI.get_idrac_local_account_attr",
    ...
    def test_create_or_modify_account_1(self, idrac_connection_user_mock, idrac_default_args, mocker):
        ...
        mocker.patch("ansible.modules.remote_management.dellemc.idrac_user.get_payload",
                     return_value={"Users.2#UserName": "test_user"})
        mocker.patch("ansible.modules.remote_management.dellemc.idrac_user.convert_payload_xml",
                     return_value=("<xml-data>", {"Users.1#UserName": "test_user"}))
        mocker.patch("ansible.modules.remote_management.dellemc.idrac_user.iDRACRedfishAPI.import_scp",
    ...
    def test_create_or_modify_account_6(self, idrac_connection_user_mock, idrac_default_args, mocker):
        ...
        mocker.patch("ansible.modules.remote_management.dellemc.idrac_user.convert_payload_xml",
                     return_value=("<xml-data>", {"Users.1#UserName": "test_user"}))
        mocker.patch("ansible.modules.remote_management.dellemc.idrac_user.iDRACRedfishAPI.invoke_request",
                     return_value={"Message": "Successfully created a request."})
        slot_id = 2
        slot_uri = "/redfish/v1/Managers/iDRAC.Embedded.1/Accounts/{0}/".format(slot_id)
        user_attr = {"User.2#UserName": "test_user"}
    ...
    def test_main(self, exc_type, idrac_connection_user_mock, idrac_default_args, mocker):
        ...
        if exc_type not in [HTTPError, SSLValidationError]:
            mocker.patch("ansible.modules.remote_management.dellemc.idrac_user.create_or_modify_account",
