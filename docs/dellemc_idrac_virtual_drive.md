...
| vd_size | no  |  |  | Size (in bytes) of the Virtual Drive. For e.g. if you want to create a virtual drive of size 1TB, then set the ```vd_size``` to ```1099511627776 (1 &ast; 1024 &ast; 1024 &ast; 1024 &ast; 1024 = 1099511627776 bytes)```. Please make sure that the 1TB of space is available on physical drives that are to be used for creating the VD. |
