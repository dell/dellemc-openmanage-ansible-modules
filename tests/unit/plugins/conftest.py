...
from ansible_collections.dellemc.openmanage.tests.unit.utils import set_module_args, exit_json, fail_json
...
def ome_connection_mock(mocker, ome_response_mock):
    connection_class_mock = mocker.patch('ansible_collections.dellemc.openmanage.plugins.modules.ome_device_info.RestOME')
...
def ome_response_mock(mocker):
    ...
    response_class_mock = mocker.patch('ansible_collections.dellemc.openmanage.plugins.module_utils.ome.OpenURLResponse', return_value=set_method_result)
...
def redfish_response_mock(mocker):
    set_method_result = {'json_data': {}}
    response_class_mock = mocker.patch('ansible_collections.dellemc.openmanage.plugins.module_utils.redfish.OpenURLResponse', return_value=set_method_result)
    response_class_mock.success = True
    response_class_mock.status_code = 200
    return response_class_mock
