...
resource_detailed_inventory = {"detailed_inventory:": {"device_id": {Constants.device_id1: None},
                                                       "device_service_tag": {Constants.device_id2: Constants.service_tag1}}}
...
class TestOmeDeviceInfo(FakeAnsibleModule):
    ...
    def test_main_detailed_inventory_success_case(self, ome_default_args, module_mock, validate_device_inputs_mock, ome_connection_mock,
                                                  get_device_resource_parameters_mock, ome_response_mock):
        ...
        ome_response_mock.json_data = {"value": [{"device_id": {"1234": "details"}}, {"device_service_tag": {Constants.service_tag1: "details"}}]}
        ome_response_mock.status_code = 200
        result = self._run_module(ome_default_args)
        assert result['changed'] is False
        assert 'device_info' in result
    ...
    params = [{"fact_subset": "basic_inventory", "system_query_options": {"device_id": [Constants.device_id1]}},
              {"fact_subset": "subsystem_health", "system_query_options": {"device_service_tag": [Constants.service_tag1]}},
              {"fact_subset": "detailed_inventory", "system_query_options": {"device_id": [Constants.device_id1], "inventory_type": "serverDeviceCards"}}]
