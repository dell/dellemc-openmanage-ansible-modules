...
class TestIdracRedfishStorageController(FakeAnsibleModule):
    ...
    def test_idrac_redfish_storage_controller_main_success_case_02(self,
        ...
        mocker.patch('ansible_collections.dellemc.openmanage.plugins.modules.idrac_redfish_storage_controller.check_id_exists')
        mocker.patch(
            'ansible_collections.dellemc.openmanage.plugins.modules.idrac_redfish_storage_controller.check_volume_array_exists')
        mocker.patch(
            'ansible_collections.dellemc.openmanage.plugins.modules.idrac_redfish_storage_controller.check_encryption_capability')
        f_module = self.get_module_mock(params=param)
        redfish_response_mock.success = True
        redfish_response_mock.headers = {"Location": "Jobs/1234"}
        redfish_default_args.update(param)
    ...
    def test_idrac_redfish_storage_controller_main_exception_case(self, exc_type, mocker,
        ...
        mocker.patch(
                'ansible_collections.dellemc.openmanage.plugins.modules.idrac_redfish_storage_controller.check_raid_service',
                side_effect=exc_type('test'))
        mocker.patch(
                'ansible_collections.dellemc.openmanage.plugins.modules.idrac_redfish_storage_controller.check_id_exists',
                side_effect=exc_type('test'))
        mocker.patch(
                'ansible_collections.dellemc.openmanage.plugins.modules.idrac_redfish_storage_controller.check_volume_array_exists',
                side_effect=exc_type('test'))
        mocker.patch(
                'ansible_collections.dellemc.openmanage.plugins.modules.idrac_redfish_storage_controller.validate_inputs',
                side_effect=exc_type('test'))
